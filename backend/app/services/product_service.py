...
class ProductService:
    ...
    # метод get_all_products получаем все продукты
    def get_all_products(self) -> ProductListResponse:  
        products = self.product_repository.get_all()
        # создаем список продуктов ответа и валидируем их 
    ...
    def get_product_by_id(self, product_id: int) -> ProductResponse:    # на вход принимаем id продукта (в int), на выход возвращаем продукт
    ...
    def get_products_by_category(self, category_id: int) -> ProductListResponse:
        ...
        products_response = [ProductResponse.model_validate(prod) for prod in products] # создаем список продуктов ответа и валидируем их 
        return ProductListResponse(products=products_response, total=len(products_response))
