...
class CartService:
    ...
    # добавляем товар в корзину и возвращаем обновленную корзину в виде словаря {product_id: quantity}
    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        ...
        if item.product_id in cart_data:    # cart_data берется из сессии (куки)
            cart_data[item.product_id] += item.quantity  # если товар уже есть в корзине, увеличиваем количество
        else:
            cart_data[item.product_id] = item.quantity 

        return cart_data

    # обновляем количество товара в корзине и возвращаем обновленную корзину в виде словаря {product_id: quantity}
    ...
    # удаляем товар из корзины и возвращаем обновленную корзину в виде словаря {product_id: quantity} 
    ...
    # возвращаем корзину в виде объекта CartResponse 
    def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:  # берем данные cart_data из сессии, получаем объект CartResponse
        if not cart_data:
